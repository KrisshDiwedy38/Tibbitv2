...
class RibbonParticle {
  ...
  update(mouseX: number, mouseY: number, time: number) {
    ...
    let targetX = this.baseX + Math.sin(time * this.speedX + this.phaseX) * 4;
    let targetY = this.baseY + Math.cos(time * this.speedY + this.phaseY) * 4;
    
    // Mouse repulsion
    let dx = this.x - mouseX;
    let dy = this.y - mouseY;
    let distSq = dx * dx + dy * dy;
    let maxDist = 120; // 120px radius
...
export default function HeroRibbon() {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Translate global client coordinates to canvas-local coordinates
    ...
    // Track mouse over the window to ensure fluid movement across UI elements overlaying the canvas
